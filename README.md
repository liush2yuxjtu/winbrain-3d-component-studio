...
脚本驱动真实 Chromium，检查 6 个页面 + 28 个独立预览页可加载、无 JavaScript 错误、无外部请求，并把 15 组「同一组件在出货页面 vs 在预览里的 computedStyle」逐属性对比。结果写入 `design-system-verification.json`，截图落在 `design-system-review/`。
