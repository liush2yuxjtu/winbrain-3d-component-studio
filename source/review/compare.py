...
import cv2
import numpy as np
import json, hashlib, html, importlib.util, os
...
for c in changed:
    ...
    rows.append({'id':c['id'],'rect':[x,y,w,h],'before':scores[0],'after':scores[1],
...
result={'source_commit':os.environ.get('GITHUB_SHA'),'baseline_commit':'bc2a884927706d66fe107d8c0c2c2b7af81ba853',
...
for row in rows:
    ...
    state='Both metrics improved' if a['mae_rgb']<b['mae_rgb'] and a['ssim_gray']>b['ssim_gray'] else 'Mixed metrics: inspect visually'
