...
const groupButtons = ["全部", ...groups]
  .map((group, index) => `<button data-group-filter="${esc(group)}"${index === 0 ? ' class="active"' : ""}>${esc(group)}</button>`)
  .join("");
